...
import { LEVEL_ORDER, getModule } from "@/lib/course";
import {
...
export function Dashboard() {
  const { courseId, meta, course } = useCourseContext();
  ...
  const levels = LEVEL_ORDER.map((level) => ({
    ...
    progress: hydrated ? getLevelProgress(courseId, level, state) : 0,
  })).filter((l) => l.modules.length > 0);

  const completedModules = hydrated
    ? getCompletedModuleCount(courseId, state)
    : 0;

  const resumeId = getResumeModuleId(courseId, state);
  const resumeModule = getModule(courseId, resumeId);
  ...
  return (
    <div className="animate-fade-in">
      <nav className="mb-6 flex items-center gap-2 text-sm text-neutral-400">
        ...
        <span>/</span>
        <span className="text-neutral-950">{meta.title}</span>
      </nav>

      {resumeModule && hydrated && (
        <Link
          href={`/courses/${courseId}/modules/${resumeId}`}
      ...
      <div className="mb-10 grid gap-px overflow-hidden rounded-xl border border-neutral-200 bg-neutral-200 sm:grid-cols-3 animate-fade-up stagger-2">
        {[
          {
            label: "Modules",
            value: String(meta.moduleCount),
            sub: `${course.modules[0]?.id}–${course.modules[course.modules.length - 1]?.id} structured`,
          },
          {
            label: "Completed",
          ...
          {
            label: "Duration",
            value: meta.id === "agentic-memory" ? "10–12" : "16–20",
            sub: "weeks suggested",
          },
        ].map((stat) => (
      ...
      <div className="mb-10 grid gap-4 md:grid-cols-2 animate-fade-up stagger-3">
        {[
          { title: "Course Format", content: meta.format },
          {
            title: "Prerequisites",
            content: (
          ...
          { title: "Suggested Pace", content: meta.pace },
        ].map((card) => (
          <article
            key={card.title}
